...
RSpec.describe Hospodar do
  vars do
    target! do
      Class.new do
        ...
        nest :main, delegate: true, base_class: Class.new {
                                                  def to_s
                                                    'main'
                                                  end
                                                } do
  ...
  it { expect(target).to respond_to(:mother_ship_steps) }
  ...
  describe '.assemble_*_struct' do
    ...
    context 'when break proc and after creation proc provided' do
      it 'returns enumerator with created objects' do
        plan = target.build_main { |b| b.two_step(3, 4) }
  ...
  context 'double inheritance' do
    ...
    describe '.assemble_*_struct' do
      ...
      context 'when break proc and after creation proc provided' do
        it 'returns enumerator with created objects' do
          plan = child_of_child.build_main { |b| b.two_step(3, 4) }
          res = plan.do_until { |_, id| id.to_sym == :one_step }.call
          expect(res.two.x).to eq 3
          expect(res.two.y).to eq 4
          expect(res).to_not respond_to(:zero)
          expect(res).to_not respond_to(:four)
